...
#include "ethercat_comm/DataElmo.h"
#include "ethercat_comm/OutputElmo.h"
...
private:
    ...
    bool flag;
    ethercat_comm::OutputElmo joint_velocity_msg;
...
public:
    ...
    void start();
    void joint_state_callback(const ethercat_comm::DataElmo::ConstPtr& joint_state_msg);
...
void franka_robot::joint_state_callback(const ethercat_comm::DataElmo::ConstPtr &joint_state_msg)
{
   //  for(int i=0;i<3;i++)  theta[i]=joint_state_msg->position[i];     
     theta[0]=joint_state_msg->PositionActualValue1;
     theta[1]=joint_state_msg->PositionActualValue2;
     theta[2]=joint_state_msg->PositionActualValue3;
...
void franka_robot::start()
{
     ...
     joint_velocity_pub=nh.advertise<ethercat_comm::OutputElmo>("writer_elmo_joint1",100);
     IMU_sub=nh.subscribe<BBL_robot::IMU_sEMG>("emg_data",1000,&franka_robot::IMU_callback,this);
     joint_state_sub=nh.subscribe<ethercat_comm::DataElmo>("reader_elmo_joint1",10,&franka_robot::joint_state_callback,this);

    //ros::Publisher pub_elmo_j1 = n.advertise<ethercat_comm::DataElmo>("reader_elmo_joint1", 1000);
    // ros::Subscriber sub = n.subscribe("writer_elmo_joint1", 1000, chatterElmoOutCallback);
...
//std::cout<<<<std::endl;
        ...
        //    if(M.determinant()>1.4579e-05){
            ...
            if(M.determinant()>0.0001){
               ...
               for(int i=0;i<3;i++)  {
                 ...
                 // if(joint_v(i)<0.02 && joint_v(i)>-0.02)  joint_v(i)=0; 
                  v_rpm[i]=round(joint_v(i)*60*101/(2*M_PI));
                  std::cout<<"the desire rpm is:"<<v_rpm[i]<<std::endl;
               }

               
               joint_velocity_msg.TargetPosition=v_rpm[0];
               joint_velocity_msg.TargetPosition2=v_rpm[1];
               joint_velocity_msg.TargetPosition3=v_rpm[2];
               joint_velocity_msg.TargetPosition5=-v_rpm[1]-v_rpm[2];
               joint_velocity_msg.Controlword=0x000f;
            ...
            //   joint_v<<0,0,0,0,0,0;
              ...
              joint_velocity_msg.TargetPosition=0;
               joint_velocity_msg.TargetPosition2=0;
               joint_velocity_msg.TargetPosition3=0;
               joint_velocity_msg.Controlword=0x000f;
              joint_velocity_pub.publish(joint_velocity_msg);
              ros::Duration(0.1).sleep();
           }
